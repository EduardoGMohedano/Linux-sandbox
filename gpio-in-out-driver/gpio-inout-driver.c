...
static ssize_t gpio_write(struct file *file, const char __user *buffer, size_t len, loff_t *offset){
    ...
    /* Set GPIO value */
    gpiod_set_value(data->gpio[0], value);
...
static ssize_t gpio_read(struct file *file, char __user *buffer, size_t len, loff_t *offset){
    ...
    value = gpiod_get_value(data->gpio[1]);
...
static int custom_gpio_probe(struct platform_device *pdev)
{
    ...
    data->gpio[0] = gpiod_get_index(&pdev->dev, NULL, 0, GPIOD_ASIS);
    if (IS_ERR(data->gpio[0])) {
        ret = PTR_ERR(data->gpio[0]);
        dev_err(&pdev->dev, "Failed to get GPIO: %d\n", ret);
        return ret;
    }

    //Input setting
    data->gpio[1] = gpiod_get_index(&pdev->dev, NULL, 1, GPIOD_ASIS); //GPIOD_ASIS keeps config read from DT overlay
    if (IS_ERR(data->gpio[1])) {
        ret = PTR_ERR(data->gpio[1]);
        dev_err(&pdev->dev, "Failed to get GPIO: %d\n", ret);
        return ret;
    }

    /* Skipping setting GPIO directions*/
    ...
    dev_info(&pdev->dev, "GPIO[%d] as out and GPIO[%d] as input\n", desc_to_gpio(data->gpio[0]), desc_to_gpio(data->gpio[1]));
